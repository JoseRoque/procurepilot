...
function detectStatus(rawText: string): VisibleOfferStatus {
	// "appears_applied" strictly requires explicit page wording; a visible offer
	// is a claim by the page, never guaranteed eligibility.
	if (APPLIED_PATTERN.test(rawText)) return "appears_applied";
	return "visible";
}

/**
 * Parses one line of visible offer/promo text into a structured VisibleOffer.
 * Conservative by design: ambiguous wording falls back to "unknown" type and
...
export function parseVisibleOffer(rawText: string): VisibleOffer {
	...
	// Credits/rebates are usually phrased "$X credit"/"$X cash back" without the
	// word "off" — fall back to the first plain dollar amount for those.
	...
	if (offerType !== "unknown" && (discountPercent !== undefined || discountCents !== undefined)) {
		confidence =
			minimumSpendCents !== undefined || offerType === "free_delivery" ? "high" : "medium";
