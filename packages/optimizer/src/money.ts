import type { MoneyFact } from "../../domain/src";
 ...
 * like a money value. All arithmetic downstream is integer cents.
 */
...
/**
 * Integer percent-of with explicit floor rounding (rounds the benefit DOWN —
 * the conservative direction for anything presented as a saving).
 */
export function percentOfCentsFloor(cents: number, percent: number): number {
	return Math.floor((cents * Math.trunc(percent)) / 100);
}
